...
export default defineConfig({
  ...
  outDir: "dist",
  clean: true,
  // depthbake-core は private ワークスペースパッケージなので npm には公開しない。
  // 外部 import のまま publish すると install 時に依存解決が失敗するため、CLI にバンドルする。
  // depthbake-runtime は公開パッケージだが、core が導出関数(masks/normals)を
  // 再エクスポートしているだけなので依存に追加せず一緒にバンドルする。
  noExternal: ["depthbake-core", "depthbake-runtime"],
